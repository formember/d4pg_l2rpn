...
class MyD4PGLearner(acme.Learner):
    """D4PG learner.
    ...
    def _step(self, sample) -> Dict[str, tf.Tensor]:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            q_t = self._target_critic_network(o_t, gumbel_softmax(self._target_policy_network(o_t)))
            ...
            dpg_a_t_gum = gumbel_softmax(dpg_a_t, hard=True)
            dpg_z_t = self._critic_network(o_t, dpg_a_t_gum)
    ...
    def step(self):
        ...
        with self._summary_writer.as_default():
            tf.summary.scalar('critic_loss', fetches['critic_loss'].numpy(), step=counts['steps'])
            tf.summary.scalar('policy_loss', fetches['policy_loss'].numpy(), step=counts['steps'])
